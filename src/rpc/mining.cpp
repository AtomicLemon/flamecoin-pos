...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (Params().MiningRequiresPeers()) {
        if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
            throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Flamecoin Core is not connected!");

        if (IsInitialBlockDownload())
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Flamecoin Core is downloading blocks...");
    }
    ...
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
        ...
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Flamecoin Core is syncing with network...");
