    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "flamecoin" is a composite category enabling all Flamecoin-related debug output
                if(ptrCategory->count(std::string("flamecoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FlamecoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FlamecoinCore
    // Mac: ~/Library/Application Support/FlamecoinCore
    // Unix: ~/.flamecoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FlamecoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FlamecoinCore";
#else
    // Unix
    return pathRet / ".flamecoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty flamecoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override flamecoin.conf
