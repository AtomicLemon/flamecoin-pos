...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"flamecoinprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"flamecoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Flamecoin Core %s\n", CLIENT_BUILD);
    ...
    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("flamecoincoreversion", CLIENT_BUILD));
