...
# Copyright (c) 2014-2017 The Flamecoin Core developers
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for flamecoind to start. This means that RPC is accessible and fully initialized.
    Raise an exception if flamecoind exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('flamecoind exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        # Create cache directories, run flamecoinds:
        for i in range(MAX_NODES):
            datadir=initialize_datadir(cachedir, i)
            args = [ os.getenv("BITCOIND", "flamecoind"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            logger.debug("initialize_chain: flamecoind started, waiting for RPC to come up")
    ...
    for i in range(num_nodes):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in flamecoin.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False, stderr=None):
    """
    Start a flamecoind and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("BITCOIND", "flamecoind")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start multiple flamecoinds, return RPC connections to them
    """
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s FLAMECOIN too low! (Should be %s FLAMECOIN)"%(str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s FLAMECOIN too high! (Should be %s FLAMECOIN)"%(str(fee), str(target_fee)))
