...
"""Flamecoin P2P network half-a-node.
...
def flamecoinhash(s):
    return flamecoin_hash.getPoWHash(s)
...
# Objects that map to flamecoind objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(flamecoinhash(r))
            self.hash = encode(flamecoinhash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in flamecoind indicates these should be deserialized as blocks
